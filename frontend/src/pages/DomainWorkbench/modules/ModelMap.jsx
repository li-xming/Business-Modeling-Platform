    ...
    if (displayMode === 'force-directed') {
      ...
      const nodeGroup = g.append('g')
        ...
        .attr('transform', (d, i) => {
          // UML类图布局：水平排列
          const columns = Math.ceil(Math.sqrt(modelNodes.length));
          const spacing = 350; // 模型之间的间距
          const x = (i % columns) * spacing + 150;
          const y = Math.floor(i / columns) * 250 + 150;
          return `translate(${x}, ${y})`;
        });
      ...
      nodeGroup.each(function(d) {
        ...
        // 计算类图高度
        const classHeight = 60 + (isPropertyExpanded ? properties.length * 25 : 0);
        ...
        if (isPropertyExpanded && properties.length > 0) {
          properties.forEach((property, index) => {
            ...
            g.append('text')
              ...
              .text(`${property.isPrimaryKey ? '+ ' : '- '}${property.name}: ${property.type}`)
              .attr('x', 15)
              .attr('y', y)
              .attr('fill', '#334155')
              .attr('font-size', '11px');
      ...
      umlLinks
        .attr('x1', d => {
          ...
          return sourceNode && !sourceG.empty() ? sourceG.node().transform.baseVal.getItem(0).matrix.e + 100 : 0;
        })
        .attr('y1', d => {
          // 获取 source 和 target 的实际 id
          const sourceId = typeof d.source === 'object' ? d.source.id : d.source;
          const sourceNode = nodes.find(n => n.id === sourceId);
          const sourceG = g.selectAll('.uml-class-node').filter((node) => node.id === sourceId);
          return sourceNode && !sourceG.empty() ? sourceG.node().transform.baseVal.getItem(0).matrix.f + 30 : 0;
        })
        .attr('x2', d => {
          // 获取 source 和 target 的实际 id
          const targetId = typeof d.target === 'object' ? d.target.id : d.target;
          ...
          return targetNode && !targetG.empty() ? targetG.node().transform.baseVal.getItem(0).matrix.e + 100 : 0;
        })
        .attr('y2', d => {
          // 获取 source 和 target 的实际 id
          const targetId = typeof d.target === 'object' ? d.target.id : d.target;
          const targetNode = nodes.find(n => n.id === targetId);
          const targetG = g.selectAll('.uml-class-node').filter((node) => node.id === targetId);
          return targetNode && !targetG.empty() ? targetG.node().transform.baseVal.getItem(0).matrix.f + 30 : 0;
        });
      ...
      linkLabels
        .attr('x', d => {
          ...
          const sourceG = g.selectAll('.uml-class-node').filter((node) => node.id === sourceId);
          const targetG = g.selectAll('.uml-class-node').filter((node) => node.id === targetId);
          const sourceX = sourceNode && !sourceG.empty() ? sourceG.node().transform.baseVal.getItem(0).matrix.e + 100 : 0;
          const targetX = targetNode && !targetG.empty() ? targetG.node().transform.baseVal.getItem(0).matrix.e + 100 : 0;
          return (sourceX + targetX) / 2;
        })
        .attr('y', d => {
          // 获取 source 和 target 的实际 id
          ...
          const sourceG = g.selectAll('.uml-class-node').filter((node) => node.id === sourceId);
          const targetG = g.selectAll('.uml-class-node').filter((node) => node.id === targetId);
          const sourceY = sourceNode && !sourceG.empty() ? sourceG.node().transform.baseVal.getItem(0).matrix.f + 30 : 0;
          const targetY = targetNode && !targetG.empty() ? targetG.node().transform.baseVal.getItem(0).matrix.f + 30 : 0;
          return (sourceY + targetY) / 2 - 5;
        });
